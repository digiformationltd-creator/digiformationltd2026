...
BEGIN
  ...
  v_preview := CASE _intent
    ...
    WHEN 'send_email_template' THEN jsonb_build_object(
      'summary','Queue transactional email','will_call','send-transactional-email',
    ...
    WHEN 'create_invoice' THEN jsonb_build_object(
      'summary','Generate invoice','will_call','generate-invoice',
      'order_id',_payload->>'order_id','customer_email',_payload->>'customer_email')
    WHEN 'create_order' THEN jsonb_build_object(
      'summary','Create order','will_call','agent-create-order',
    ...
    WHEN 'update_order_status' THEN jsonb_build_object(
      'summary','Update order status','will_update_table','client_orders',
