// Phone number normalisation helpers used across checkout forms.
// We normalise to E.164 (+<country><number>) using the customer's selected
// country of residence to infer a dialing code when the number was typed
// in local format (e.g. "03001234567" for Pakistan).
...
export const COUNTRY_DIAL_CODES: Record<string, string> = {
  ...
  "Italy": "39", "Ivory Coast": "225", "Jamaica": "1876", "Japan": "81", "Jordan": "962",
  ...
  "Liechtenstein": "423", "Lithuania": "370", "Luxembourg": "352", "Macau": "853", "Madagascar": "261",
  ...
  "Romania": "40", "Russia": "7", "Rwanda": "250", "Saint Kitts and Nevis": "1869",
...
/**
 * Normalise a user-entered phone number into E.164 format.
 * - "+44 7438 351454"   → "+447438351454"
 * - "00923001234567"    → "+923001234567"
 * - "03001234567" + PK  → "+923001234567"
 * - "(415) 555-1212" + US → "+14155551212"
 */
export function normalizePhoneToE164(raw: string | null | undefined, country?: string | null): string | null {
  if (!raw) return null;
  const trimmed = String(raw).trim();
  if (!trimmed) return null;

  // Already starts with `+` → keep digits after the +.
  ...
  // Local format — try to use country dialing code.
  const dial = country ? COUNTRY_DIAL_CODES[country.trim()] : undefined;
  if (dial) {
  ...
  // Fallback — return digits as-is with a leading + so it is at least uniform.
  return digits.length >= 6 ? `+${digits}` : null;
}
