        ...
        {tab === "orders" && (
          <div className="space-y-3">
            <div className="text-sm text-muted-foreground px-1">
              ...
              <span className="ml-2 italic">Orders are created automatically when a client places one. Order numbers are generated by the system.</span>
            </div>
            {orders.length === 0 && (
...
const AffiliatesPanel = () => {
  ...
  const load = async () => {
    ...
    setRows(data || []);
    // pull profile names/emails for these users
    ...
    if (ids.length) {
      ...
      (profs || []).forEach((p) => {
        m[p.user_id] = { name: p.full_name || "—", email: p.email || "—" };
      });
      setNameMap(m);
    }
    // recent commissions across all
  ...
  const updateStatus = async (userId: string, status: string) => {
    ...
    else { toast.success(`Status updated to ${status}`); load(); }
  };

  const setCommissionRate = async (userId: string, rate: number) => {
    const { error } = await (supabase as any)
      .from("affiliate_profiles")
      .update({ commission_per_sale_gbp: rate })
      .eq("user_id", userId);
    if (error) toast.error(error.message);
    else { toast.success("Commission rate updated"); load(); }
  };
  ...
  if (loading) return <div className="glass rounded-2xl p-10 text-center"><Loader2 className="w-5 h-5 animate-spin inline" /></div>;
    ...
    <div className="space-y-6">
      <div className="glass rounded-2xl overflow-hidden">
        <div className="p-4 border-b border-border/40 flex items-center justify-between">
          <div>
            ...
            <p className="text-xs text-muted-foreground">Approve, set commission rates, view stats</p>
          </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody>
              {rows.map((r) => {
                ...
                return (
                  <tr key={r.user_id} className="border-t border-border/40">
                    ...
                    <td className="p-3">
                      <Badge variant={r.status === "approved" ? "default" : r.status === "suspended" ? "destructive" : "secondary"} className="capitalize">
                        {r.status}
                      </Badge>
                    </td>
                    <td className="p-3 text-right">{r.total_clicks}</td>
                    <td className="p-3 text-right">{r.total_signups}</td>
                    <td className="p-3 text-right">{r.total_paid_orders}</td>
                    <td className="p-3 text-right">£{Number(r.pending_commission_gbp).toFixed(2)}</td>
                    <td className="p-3 text-right">£{Number(r.lifetime_commission_gbp).toFixed(2)}</td>
                    <td className="p-3 text-right">
                      <Input
                        type="number"
                        defaultValue={r.commission_per_sale_gbp}
                        className="h-7 w-16 text-right"
                        onBlur={(e) => {
                          const v = parseFloat(e.target.value);
                          if (!isNaN(v) && v !== Number(r.commission_per_sale_gbp)) setCommissionRate(r.user_id, v);
                        }}
                      />
                    </td>
                    <td className="p-3 text-right whitespace-nowrap">
                      <div className="flex gap-1 justify-end flex-wrap">
                        {r.status !== "approved" && (
                          <Button size="sm" variant="default" onClick={() => updateStatus(r.user_id, "approved")}>Approve</Button>
                        )}
                        {r.status === "approved" && (
                          <Button size="sm" variant="outline" onClick={() => updateStatus(r.user_id, "suspended")}>Suspend</Button>
                        )}
                        {r.status === "suspended" && (
                          <Button size="sm" variant="default" onClick={() => updateStatus(r.user_id, "approved")}>Reactivate</Button>
                        )}
                        <Button size="sm" variant="ghost" onClick={() => setExpanded(expanded === r.user_id ? null : r.user_id)}>
                          {expanded === r.user_id ? "Hide" : "Payout"}
                        </Button>
                      </div>
                      {expanded === r.user_id && (
                        <div className="text-xs text-left mt-2 p-2 bg-muted/20 rounded">
                          <div><strong>Method:</strong> {r.payout_method || "—"}</div>
                          <div><strong>Details:</strong> {r.payout_details || "—"}</div>
                        </div>
                      )}
                    </td>
                  </tr>
              ...
              {rows.length === 0 && (
                <tr><td colSpan={10} className="p-8 text-center text-muted-foreground">No affiliate partners yet</td></tr>
              )}
            </tbody>
          </table>
        </div>
      ...
      <div className="glass rounded-2xl overflow-hidden">
        <div className="p-4 border-b border-border/40">
          ...
          <p className="text-xs text-muted-foreground">Approve and mark as paid once payouts go out</p>
        </div>
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead className="bg-muted/30 text-left">
            ...
            <tbody>
              {commissions.map((c) => {
                const u = nameMap[c.affiliate_user_id];
                return (
                  <tr key={c.id} className="border-t border-border/40">
                    <td className="p-3 whitespace-nowrap">{new Date(c.created_at).toLocaleDateString()}</td>
                    <td className="p-3">{u?.name || c.affiliate_user_id.slice(0, 8)}</td>
                    ...
                    <td className="p-3 text-right font-semibold">£{Number(c.commission_gbp).toFixed(2)}</td>
                    <td className="p-3">
                      <Badge variant={c.status === "paid" ? "default" : c.status === "cancelled" ? "destructive" : "secondary"} className="capitalize">
                        {c.status}
                      </Badge>
                    </td>
                    <td className="p-3 text-right whitespace-nowrap">
                      <div className="flex gap-1 justify-end">
                        ...
                        {(c.status === "pending" || c.status === "approved") && (
                          <Button size="sm" variant="default" onClick={() => markCommissionPaid(c.id)}>Mark Paid</Button>
                        )}
                        {c.status !== "paid" && c.status !== "cancelled" && (
                          <Button size="sm" variant="ghost" onClick={() => cancelCommission(c.id)}><Trash2 className="w-3 h-3" /></Button>
                        )}
                      </div>
                    </td>
                  </tr>
              ...
              {commissions.length === 0 && (
                <tr><td colSpan={8} className="p-8 text-center text-muted-foreground">No commissions yet</td></tr>
              )}
            </tbody>
          </table>
        </div>
